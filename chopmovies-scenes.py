    ...
    for i,line in enumerate(lines):
      ...
      if len(bold) > 0:
        ...
        if re.search(r"(CUT|FADE|BACK|DISSOLVE) (TO|IN|OUT)",bold[0][1]):
          types[i] = "r"
        if re.search(r"(INT|EXT)[ .]",bold[0][1]) or re.search(r"(INTERIOR|EXTERIOR)",bold[0][1]):
          types[i] = "n"
        text = bold[0][1].rstrip(" ")
        if re.match(r"\(.*\)\r?$",text) and not types[i]=="n":
          types[i] = "p"
        lines[i] = space+text
        ...
        if re.match(r"\(.*?\)\r?$",line_match[0][1]):
          ...
          types[i] = "d"
      if len(condensedTypes) > 0 and not condensedTypes[-1]=="s" and condensedTypes[-1] == types[i]:
